...
let nextId = {productos:7, categorias:5, proveedores:4};
...
function saveItem(){
  ...
  if(activeTab==='productos'){
    const obj={id:editingId||nextId.productos++,nombre:f1,
    ...
    if(editingId){const i=data.productos.findIndex(x=>x.id===editingId);data.productos[i]=obj}
    else data.productos.push(obj);
  } else if(activeTab==='categorias'){
